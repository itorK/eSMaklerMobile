    ...
    model: function(params) {
        ...
        db.readTransaction(function(t) {
            ...
            t.executeSql('SELECT nazwa, wartosc from parametry', [], function(t, data) {
                ...
                /* var data1 = [];
                 that.store.find('param').then(function(items) {

                 items.map(function(item) {
                 data1.push(item.getProperties(['nazwa']));
                 console.log("dwa :" + item.getProperties(['nazwa']) + "omg");
                 });
                 })
                 var cos = that.store.find('param', {nazwa: "server"});
                 */
                // console.log("jeden :" + );
                /* var data1 = that.store.find('param').then(function() {
                 console.log("dwa :" + data1.get('length') + "omg");
                 })*/


                // var parametry = this.store.all('param');
                if (!server) {
                ...
                /*
                 ...
                 App.MyAdapter.reopen({
                 host: server
                 // host: 'http://10.20.70.43:8084/WebApplication2/test'
                 // 'http://10.20.70.43:8084/WebApplication2/test'
                 });
                 App.MyAdapter.reopenClass({
                 host: server
                 // host: 'http://10.20.70.43:8084/WebApplication2/test'
                 // 'http://10.20.70.43:8084/WebApplication2/test'
                 });
                 ...
                 App.ApplicationAdapter.reopenClass({
                 host: server
                 // host: 'http://10.20.70.43:8084/WebApplication2/test'
                 // 'http://10.20.70.43:8084/WebApplication2/test'
                 });
